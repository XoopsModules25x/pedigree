...
class Utility extends Common\SysUtility
{
    ...
    */
    ...
    public static function pups($oid, $gender)
    {
        ...
        if (0 == $gender) {
            $sqlquery = 'SELECT d.id AS d_id, d.pname AS d_pname, d.roft AS d_roft, d.* FROM ' . $GLOBALS['xoopsDB']->prefix('pedigree_registry') . ' d LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('pedigree_registry') . ' f ON d.father = f.id LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix(
                    'pedigree_registry'
                ) . ' m ON d.mother = m.id WHERE d.father=' . $oid . ' ORDER BY d.pname';
        } else {
            $sqlquery = 'SELECT d.id AS d_id, d.pname AS d_pname, d.roft AS d_roft, d.* FROM ' . $GLOBALS['xoopsDB']->prefix('pedigree_registry') . ' d LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix('pedigree_registry') . ' f ON d.father = f.id LEFT JOIN ' . $GLOBALS['xoopsDB']->prefix(
                    'pedigree_registry'
                ) . ' m ON d.mother = m.id WHERE d.mother=' . $oid . ' ORDER BY d.pname';
        }
        ...
        foreach ($fields as $i => $iValue) {
            ...
            $userField   = new Pedigree\Field($fields[$i], $animal->getConfig());
            $fieldType   = $userField->getSetting('fieldtype');
            $className   = '\XoopsModules\\' . \ucfirst(mb_strtolower(\basename(\dirname(__DIR__)))) . '\\' . $fieldType;
            $fieldObject = new $className($userField, $animal);
