...
class phpthumb_filters
{
            ...
            for ($x = 0; $x < ImageSX($gdimg); ++$x) {
                for ($y = 0; $y < ImageSY($gdimg); ++$y) {
                    if ($PixelMap[$x][$y]['alpha'] < 127) {
                        $thisColor = phpthumb_functions::ImageColorAllocateAlphaSafe(
                            $gdimg_dropshadow_temp,
                            $PixelMap[$x][$y]['red'],
                            $PixelMap[$x][$y]['green'],
                            $PixelMap[$x][$y]['blue'],
                            $PixelMap[$x][$y]['alpha']
    ...
    public function Frame(&$gdimg, $frame_width, $edge_width, $hexcolor_frame, $hexcolor1, $hexcolor2)
    {
        ...
        for ($i = 0; $i < $edge_width; ++$i) {
            ...
            ImageLine(
                $gdimg,
                ImageSX($gdimg) - $frame_width - $edge_width - $i,
                ImageSY($gdimg) - $frame_width - $edge_width - $i,
                ImageSX($gdimg) - $frame_width - $edge_width - $i,
                $frame_width + $edge_width + $i,
                $color1
            ); // right
            ImageLine(
                $gdimg,
    ...
    public function ImageBorder(&$gdimg, $border_width, $radius_x, $radius_y, $hexcolor_border)
    {
        ...
        list($new_width, $new_height) = phpthumb_functions::ProportionalResize(
            $output_width,
            $output_height,
            $output_width - max($border_width * 2, $radius_x),
            $output_height - max($border_width * 2, $radius_y)
        );
...
//exit;
        if ($gd_border_canvas = phpthumb_functions::ImageCreateFunction($output_width, $output_height)) {
            ...
            for ($i = 0; $i < $border_width; ++$i) {
                ...
                ImageLine(
                    $gd_border_canvas,
                    floor($offset_x / 2) + $radius_x,
                    $output_height - 1 - $i,
                    $output_width - $radius_x - ceil($offset_x / 2),
                    $output_height - 1 - $i,
                    $color_border
                ); // bottom
            ...
            if ($radius_x && $radius_y) {
                ...
                for ($thickness_offset = 0; $thickness_offset < $border_width; ++$thickness_offset) {
                    ...
                    ImageArc(
                        $gd_border_canvas,
                        $output_width - $radius_x - 1 - ceil($offset_x / 2),
                        $output_height - $thickness_offset - $radius_y,
                        $radius_x * 2,
                        $radius_y * 2,
                        0,
                        90,
            ...
            $this->phpThumbObject->ImageResizeFunction(
                $gd_border_canvas,
                $gdimg,
                floor(($output_width - $new_width) / 2),
                round(($output_height - $new_height) / 2),
                0,
                0,
                $new_width,
                $new_height,
                ...
                if (ImageColorTransparent($gdimg_source) >= 0) {
                    ...
                    if (!function_exists('ImageIsTrueColor') || !ImageIsTrueColor($gdimg_source)) {
                        ...
                        ImageFilledRectangle(
                            ...
                            phpthumb_functions::ImageHexColorAllocate($gdimg_newsrc, $config_background_hexcolor)
                        );
        ...
        for ($x = 0; $x < $ImageSX; ++$x) {
            for ($y = 0; $y < $ImageSY; ++$y) {
                ...
                $newColor     = phpthumb_functions::ImageColorAllocateAlphaSafe(
                    $gdimg,
                    (~$currentPixel['red'] & 0xFF),
                    (~$currentPixel['green'] & 0xFF),
                    (~$currentPixel['blue'] & 0xFF),
                    $currentPixel['alpha']
                );
                ImageSetPixel($gdimg, $x, $y, $newColor);
            }
        ...
        if ($gdimg_cornermask_triple = phpthumb_functions::ImageCreateFunction($radius_x * 6, $radius_y * 6)) {
            if ($gdimg_cornermask = phpthumb_functions::ImageCreateFunction(ImageSX($gdimg), ImageSY($gdimg))) {
                ...
                ImageCopyResampled(
                    ...
                    ImageSX($gdimg) - $radius_x,
                    ImageSY($gdimg) - $radius_y,
                    $radius_x * 3,
                    $radius_y * 3,
                    $radius_x,
                    $radius_y,
        ...
        if (@is_readable($ttffont) && is_file($ttffont)) {
            ...
            if ($alignment == '*') {
                ...
                foreach ($textlines as $dummy => $line) {
                    ...
                    //ImageTTFtext($gdimg, $size, $angle, $text_origin_x, $text_origin_y, $letter_color_text, $ttffont, $text);
                    $this->DebugMessage(
                        'WatermarkText() calling ImageTTFtext($gdimg, ' . $size . ', ' . $angle . ', ' . $text_origin_x . ', ' . ($text_origin_y + $y_offset) . ', $letter_color_text, ' . $ttffont
                        . ', ' . $line . ')',
                        __FILE__,
                        __LINE__
                    );
            ...
            if ($img_watermark = phpthumb_functions::ImageCreateFunction($text_width, $text_height)) {
                ...
                $this->DebugMessage(
                    'WatermarkText() calling ImageFilledRectangle($img_watermark, 0, 0, ' . ImageSX($img_watermark) . ', ' . ImageSY($img_watermark) . ', $text_color_background)',
                    __FILE__,
                    __LINE__
                );
                ...
                foreach ($textlines as $key => $line) {
                    ...
                    $this->DebugMessage(
                        'WatermarkText() calling ImageString($img_watermark, ' . $size . ', ' . $x_offset . ', ' . ($key * ImageFontHeight($size)) . ', ' . $line . ', $text_color_watermark)',
                        __FILE__,
                        __LINE__
                    );
                    ...
                    if ($angle && $img_watermark_mask) {
                        $this->DebugMessage(
                            'WatermarkText() calling ImageString($img_watermark_mask, ' . $size . ', ' . $x_offset . ', ' . ($key * ImageFontHeight($size)) . ', ' . $text . ', $mask_color_watermark)',
                            __FILE__,
                            __LINE__
                        );
                ...
                $this->DebugMessage(
                    'WatermarkText() calling phpthumb_filters::WatermarkOverlay($gdimg, $img_watermark, ' . ($originOffsetX . 'x' . $originOffsetY) . ', ' . $opacity . ', 0)',
                    __FILE__,
                    __LINE__
                );
