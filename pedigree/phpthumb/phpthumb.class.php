...
class phpthumb
{
    ...
    function setSourceFilename($sourceFilename)
    {
        ...
        if (is_null($this->config_output_format)) {
            ...
            if (preg_match('#^[a-z]{3,4}$#', $sourceFileExtension)) {
                ...
                $this->DebugMessage(
                    'setSourceFilename(' . $sourceFilename . ') did NOT set $this->config_output_format to "' . $sourceFileExtension . '" because it did not seem like an appropriate image format',
                    __FILE__,
                    __LINE__
                );
            }
    ...
    function setSourceData($rawImageData, $sourceFilename = '')
    {
        ...
        $this->DebugMessage(
            'setSourceData() setting $this->rawImageData (' . strlen($this->rawImageData) . ' bytes; magic="' . substr($this->rawImageData, 0, 4) . '" (' . phpthumb_functions::HexCharDisplay(
        ...
        if ($this->config_cache_source_enabled) {
            ...
            $this->DebugMessage(
                'setSourceData() attempting to save source image to "' . $this->config_cache_source_directory . DIRECTORY_SEPARATOR . urlencode($sourceFilename) . '"',
                __FILE__,
                __LINE__
            );
...
//		}
    ...
    {
        $this->DebugMessage(
            'CleanUpCacheDirectory() set to purge (' . (is_null($this->config_cache_maxage) ? 'NULL' : number_format($this->config_cache_maxage / 86400, 1)) . ' days; ' . (is_null(
                $this->config_cache_maxsize
            ) ? 'NULL' : number_format($this->config_cache_maxsize / 1048576, 2)) . ' MB; ' . (is_null($this->config_cache_maxfiles) ? 'NULL' : number_format($this->config_cache_maxfiles))
            . ' files)',
            __FILE__,
            __LINE__
        );
        ...
        if (($this->config_cache_maxage > 0) || ($this->config_cache_maxsize > 0) || ($this->config_cache_maxfiles > 0)) {
            ...
            if ($this->config_cache_maxfiles > 0) {
                ...
                $this->DebugMessage(
                    'CleanUpCacheDirectory() purged ' . count($DeletedKeys['maxfiles']) . ' files based on (config_cache_maxfiles=' . $this->config_cache_maxfiles . ')',
                    __FILE__,
                    __LINE__
                );
            ...
            if ($this->config_cache_maxsize > 0) {
                ...
                $this->DebugMessage(
                    'CleanUpCacheDirectory() purged ' . count($DeletedKeys['maxsize']) . ' files based on (config_cache_maxsize=' . $this->config_cache_maxsize . ')',
                    __FILE__,
                    __LINE__
                );
    ...
    function setOutputFormat()
    {
        ...
        if (phpthumb_functions::CaseInsensitiveInArray($this->config_output_format, $AvailableImageOutputFormats)) {
            ...
            $this->DebugMessage(
                '$this->thumbnailFormat staying as "' . $this->thumbnailFormat . '" because $this->config_output_format (' . strtolower($this->config_output_format)
                . ') is not in $AvailableImageOutputFormats',
                __FILE__,
                __LINE__
            );
        }
        if ($this->f && (phpthumb_functions::CaseInsensitiveInArray($this->f, $AvailableImageOutputFormats))) {
            ...
            $this->DebugMessage(
                '$this->thumbnailFormat staying as "' . $this->thumbnailFormat . '" because $this->f (' . strtolower($this->f) . ') is not in $AvailableImageOutputFormats',
                __FILE__,
                __LINE__
            );
        }
    ...
    function ResolveFilenameToAbsolute($filename)
    {
        ...
        if (preg_match('#^[a-z0-9]+\:/{1,2}#i', $filename)) {
            ...
            if (@is_readable($filename) && !@is_readable($this->config_document_root . $filename)) {
                ...
                if ($ApacheLookupURIarray = phpthumb_functions::ApacheLookupURIarray($filename)) {
                    ...
                    if (@is_readable($AbsoluteFilename)) {
                        $this->DebugMessage(
                            'phpthumb_functions::ApacheLookupURIarray() failed for "' . $filename . '", but the correct filename (' . $AbsoluteFilename
                            . ') seems to have been resolved with realpath($filename)',
                            __FILE__,
                            __LINE__
                        );
                    } elseif (is_dir(dirname($AbsoluteFilename))) {
                        $this->DebugMessage(
                            'phpthumb_functions::ApacheLookupURIarray() failed for "' . dirname($filename) . '", but the correct directory (' . dirname($AbsoluteFilename)
                            . ') seems to have been resolved with realpath(.)',
                            __FILE__,
                            __LINE__
                        );
                    } else {
                ...
                if (preg_match('#^' . preg_quote($this->config_document_root) . '#', $filename)) {
                    $AbsoluteFilename = $filename;
                    $this->DebugMessage(
                        'ResolveFilenameToAbsolute() NOT prepending $this->config_document_root (' . $this->config_document_root . ') to $filename (' . $filename
                    ...
                    $this->DebugMessage(
                        'ResolveFilenameToAbsolute() prepending $this->config_document_root (' . $this->config_document_root . ') to $filename (' . $filename . ') resulting in ($AbsoluteFilename = "'
                        . $AbsoluteFilename . '")',
                        __FILE__,
                        __LINE__
                    );
            ...
            $AbsoluteFilename = $this->config_document_root . preg_replace('#[/\\\\]#', DIRECTORY_SEPARATOR, dirname(@$_SERVER['PHP_SELF'])) . DIRECTORY_SEPARATOR . preg_replace(
                    '#[/\\\\]#',
                    DIRECTORY_SEPARATOR,
                    $filename
                );
            ...
            if (substr(dirname(@$_SERVER['PHP_SELF']), 0, 2) == '/~') {
                if ($ApacheLookupURIarray = phpthumb_functions::ApacheLookupURIarray(dirname(@$_SERVER['PHP_SELF']))) {
                    ...
                    if (@is_readable($AbsoluteFilename)) {
                        $this->DebugMessage(
                            'phpthumb_functions::ApacheLookupURIarray() failed for "' . dirname(@$_SERVER['PHP_SELF']) . '", but the correct filename (' . $AbsoluteFilename
                            . ') seems to have been resolved with realpath(.)/$filename',
                            __FILE__,
                            __LINE__
                        );
                        ...
                        $this->DebugMessage(
                            'phpthumb_functions::ApacheLookupURIarray() failed for "' . dirname(@$_SERVER['PHP_SELF']) . '", but the correct directory (' . dirname($AbsoluteFilename)
                            . ') seems to have been resolved with realpath(.)',
                            __FILE__,
                            __LINE__
                        );
        ...
        if (!$this->config_allow_src_above_docroot && !preg_match('#^' . preg_quote(str_replace(DIRECTORY_SEPARATOR, '/', realpath($this->config_document_root))) . '#', $AbsoluteFilename)) {
            $this->DebugMessage(
                '!$this->config_allow_src_above_docroot therefore setting "' . $AbsoluteFilename . '" (outside "' . realpath($this->config_document_root) . '") to null',
                __FILE__,
                __LINE__
            );
        ...
        if (is_null($commandline)) {
            ...
            if ($this->config_imagemagick_path && ($this->config_imagemagick_path != realpath($this->config_imagemagick_path))) {
                if (@is_executable(realpath($this->config_imagemagick_path))) {
                    $this->DebugMessage(
                        'Changing $this->config_imagemagick_path (' . $this->config_imagemagick_path . ') to realpath($this->config_imagemagick_path) (' . realpath($this->config_imagemagick_path)
                        . ')',
                        __FILE__,
                        __LINE__
                    );
                    ...
                    $this->DebugMessage(
                        'Leaving $this->config_imagemagick_path as (' . $this->config_imagemagick_path . ') because !is_execuatable(realpath($this->config_imagemagick_path)) (' . realpath(
            ...
            $this->DebugMessage(
                'file_exists_ignoreopenbasedir(' . $this->config_imagemagick_path . ') = ' . intval($this->file_exists_ignoreopenbasedir($this->config_imagemagick_path)),
                __FILE__,
                __LINE__
            );
            ...
            if ($this->file_exists_ignoreopenbasedir($this->config_imagemagick_path)) {
                ...
                if ($this->iswindows) {
                    $commandline
                        = substr($this->config_imagemagick_path, 0, 2) . ' && cd ' . escapeshellarg(str_replace('/', DIRECTORY_SEPARATOR, substr(dirname($this->config_imagemagick_path), 2))) . ' && '
                ...
                if ($which_convert && ($which_convert{0} == '/') && $this->file_exists_ignoreopenbasedir($which_convert)) {
                    ...
                    $this->DebugMessage(
                        'ImageMagickThumbnailToGD() aborting because cannot find convert in $this->config_imagemagick_path (' . $this->config_imagemagick_path . '), and `which convert` returned ('
                        . $which_convert . ')',
                        __FILE__,
                        __LINE__
                    );
    ...
    function SourceDataToTempFile()
    {
        if ($IMtempSourceFilename = $this->phpThumb_tempnam()) {
            ...
            if ($fp_tempfile) {
                ...
                $this->DebugMessage(
                    'ImageMagickThumbnailToGD() setting $this->sourceFilename to "' . $IMtempSourceFilename . '" from $this->rawImageData (' . strlen($this->rawImageData) . ' bytes)',
                    __FILE__,
                    __LINE__
                );
            } else {
                $this->DebugMessage(
                    'ImageMagickThumbnailToGD() FAILED setting $this->sourceFilename to "' . $IMtempSourceFilename . '" (failed to open for writing: "' . $tempfile_open_error . '")',
                    __FILE__,
                    __LINE__
                );
            }
...
// newer versions of ImageMagick require -repage <geometry>
                    ...
                    switch ($command) {
                        ...
                        case 'usm':
                            if ($this->ImageMagickSwitchAvailable('unsharp')) {
                                ...
                                $commandline .= ' -unsharp ' . escapeshellarg(
                                        number_format(($radius * 2) - 1, 2, '.', '') . 'x1+' . number_format($amount / 100, 2, '.', '') . '+' . number_format($threshold / 100, 2, '.', '')
                                    );
                                $successfullyProcessedFilters[] = $filterkey;
                            }
                            break;
                                    ...
                                    if ($width && !$rX && !$rY) {
                                        ...
                                        if (preg_match('# \\-crop "([0-9]+)x([0-9]+)\\+0\\+0" #', $commandline, $matches)) {
                                            $commandline = str_replace(
                                                ' -crop "' . $matches[1] . 'x' . $matches[2] . '+0+0" ',
                                                ' -crop ' . escapeshellarg(($matches[1] - (2 * $width)) . 'x' . ($matches[2] - (2 * $width)) . '+0+0') . ' ',
                                                $commandline
                                            );
                                        } elseif (preg_match('# \\-' . $IMresizeParameter . ' "([0-9]+)x([0-9]+)" #', $commandline, $matches)) {
                                            $commandline = str_replace(
                                                ' -' . $IMresizeParameter . ' "' . $matches[1] . 'x' . $matches[2] . '" ',
                                                ' -' . $IMresizeParameter . ' ' . escapeshellarg(($matches[1] - (2 * $width)) . 'x' . ($matches[2] - (2 * $width))) . ' ',
                                                $commandline
                                            );
                                        }
                                        $successfullyProcessedFilters[] = $filterkey;
                ...
                $commandline .= ' ' . escapeshellarg(
                        preg_replace('#[/\\\\]#', DIRECTORY_SEPARATOR, $this->sourceFilename) . (($outputFormat == 'gif') ? '' : '[' . intval($this->sfn) . ']')
                ...
                if (!@file_exists($IMtempfilename) || !@filesize($IMtempfilename)) {
                    ...
                    $this->DebugMessage(
                        'GetImageSize(' . $IMtempfilename . ') returned [w=' . $getimagesize_imresized[0] . ';h=' . $getimagesize_imresized[1] . ';f=' . $getimagesize_imresized[2] . ']',
                        __FILE__,
                        __LINE__
                    );
                    if (($this->config_max_source_pixels > 0) && (($getimagesize_imresized[0] * $getimagesize_imresized[1]) > $this->config_max_source_pixels)) {
                        $this->DebugMessage(
                            'skipping ImageMagickThumbnailToGD::' . $ImageCreateFunction . '() because IM output is too large (' . $getimagesize_imresized[0] . 'x' . $getimagesize_imresized[0] . ' = '
                        ...
                        $this->DebugMessage(
                            'ImageMagickThumbnailToGD::' . $ImageCreateFunction . '() succeeded, $this->gdimg_source is now (' . $this->source_width . 'x' . $this->source_height . ')',
                            __FILE__,
                            __LINE__
                        );
    ...
    function AntiOffsiteLinking()
    {
        ...
        if ($allow && $this->config_nooffsitelink_enabled && (@$_SERVER['HTTP_REFERER'] || $this->config_nooffsitelink_require_refer)) {
            ...
            if (!$this->OffsiteDomainIsAllowed(@$parsed_url['host'], $this->config_nooffsitelink_valid_domains)) {
                ...
                $this->ErrorImage(
                    'AntiOffsiteLinking() - "' . @$parsed_url['host'] . '" is NOT in $this->config_nooffsitelink_valid_domains (' . implode(';', $this->config_nooffsitelink_valid_domains) . ')'
                );
            } else {
                $this->DebugMessage(
                    'AntiOffsiteLinking() - "' . @$parsed_url['host'] . '" is in $this->config_nooffsitelink_valid_domains (' . implode(';', $this->config_nooffsitelink_valid_domains) . ')',
                    __FILE__,
                    __LINE__
                );
            }
        ...
        if ($allow && $this->config_nohotlink_enabled && preg_match('#^(f|ht)tps?\://#i', $this->src)) {
            ...
            if (!$this->OffsiteDomainIsAllowed(@$parsed_url['host'], $this->config_nohotlink_valid_domains)) {
                ...
                $this->DebugMessage(
                    'AntiOffsiteLinking() - "' . $parsed_url['host'] . '" is NOT in $this->config_nohotlink_valid_domains (' . implode(';', $this->config_nohotlink_valid_domains) . ')',
                    __FILE__,
                    __LINE__
                );
            } else {
                $this->DebugMessage(
                    'AntiOffsiteLinking() - "' . $parsed_url['host'] . '" is in $this->config_nohotlink_valid_domains (' . implode(';', $this->config_nohotlink_valid_domains) . ')',
                    __FILE__,
                    __LINE__
                );
            }
        ...
        if ($erase) {
            ...
            if (is_file($this->config_nooffsitelink_watermark_src)) {
                ...
                $rowcounter = 0;
                $this->DebugMessage(
                    'AntiOffsiteLinking() writing ' . count($nohotlink_text_array) . ' lines of text "' . $message . '" (in #' . $this->config_error_textcolor . ') on top of image',
                    __FILE__,
                    __LINE__
                );
                foreach ($nohotlink_text_array as $textline) {
                    ...
                    ImageString(
                        $this->gdimg_output,
                        $this->config_error_fontsize,
                        $leftoffset,
                        $topoffset + (++$rowcounter * ImageFontHeight($this->config_error_fontsize)),
                        $textline,
        ...
        switch ($this->thumbnailFormat) {
            ...
            case 'gif':
                ...
                if ($CurrentImageColorTransparent == -1) {
                    ...
                    $this->DebugMessage(
                        'skipping AlphaChannelFlatten() because ($this->thumbnailFormat == "' . $this->thumbnailFormat . '") and ImageColorTransparent returned "' . $CurrentImageColorTransparent
                        . '"',
                        __FILE__,
                        __LINE__
                    );
            ...
            foreach ($this->fltr as $filtercommand) {
                ...
                switch ($command) {
                            ...
                            if ($img_watermark = $this->ImageCreateFromStringReplacement($WatermarkImageData)) {
                                ...
                                if ($underlay) {
                                    ...
                                    if ($img_watermark_resized = phpthumb_functions::ImageCreateFunction(ImageSX($this->gdimg_output), ImageSY($this->gdimg_output))) {
                                        ...
                                        $this->ImageResizeFunction(
                                            $img_watermark_resized,
                                            $img_watermark,
                                            0,
                                            0,
                                            0,
                                            0,
                                            ImageSX($img_watermark_resized),
                                            ImageSY($img_watermark_resized),
                                            ImageSX($img_watermark),
                                        ...
                                        if ($img_source_resized = phpthumb_functions::ImageCreateFunction($resized_x, $resized_y)) {
                                            ...
                                            $this->ImageResizeFunction(
                                                ...
                                                0,
                                                0,
                                                ImageSX($img_source_resized),
                                                ImageSY($img_source_resized),
                                                ImageSX($this->gdimg_output),
                                    ...
                                    if (($scale > 1) || ($scale < 1)) {
                                        if ($img_watermark2 = phpthumb_functions::ImageCreateFunction($scale * ImageSX($img_watermark), $scale * ImageSY($img_watermark))) {
                                            ...
                                            $this->ImageResizeFunction(
                                                ...
                                                0,
                                                0,
                                                ImageSX($img_watermark2),
                                                ImageSY($img_watermark2),
                                                ImageSX($img_watermark),
                                                ImageSY($img_watermark)
                                            );
                                            ...
                                            $this->DebugMessage(
                                                'ImageCreateFunction(' . ($scale * ImageSX($img_watermark)) . ', ' . ($scale * ImageSX($img_watermark)) . ') failed',
                                                __FILE__,
                                                __LINE__
                                            );
                                        }
                    ...
                    case 'size': // Resize
                        ...
                        $this->DebugMessage(
                            'Scaling watermark (' . ($stretch ? 'with' : 'without') . ' stretch) by a factor of "' . number_format($scale_x, 4) . ' x ' . number_format($scale_y, 4) . '"',
                            __FILE__,
                            __LINE__
                        );
    ...
    {
        $this->DebugMessage(
            'CalculateThumbnailDimensions() starting with [W,H,sx,sy,sw,sh] initially set to [' . $this->source_width . ',' . $this->source_height . ',' . $this->sx . ',' . $this->sy . ',' . $this->sw
            . ',' . $this->sh . ']',
            __FILE__,
            __LINE__
        );
...
//echo $this->thumbnailCropH.'<hr>';
        ...
        $this->DebugMessage(
            'CalculateThumbnailDimensions() starting with [x,y,w,h] initially set to [' . $this->thumbnailCropX . ',' . $this->thumbnailCropY . ',' . $this->thumbnailCropW . ','
            . $this->thumbnailCropH . ']',
            __FILE__,
            __LINE__
        );
    ...
    function ExtractEXIFgetImageSize()
    {
        ...
        if (is_resource($this->gdimg_source)) {
            ...
            if ($this->SourceImageIsTooLarge($this->source_width, $this->source_height)) {
                $this->DebugMessage(
                    'NOT bypassing EXIF and GetImageSize sections because source image is too large for GD (' . $this->source_width . 'x' . $this->source_width . '=' . ($this->source_width
                        * $this->source_height * 5) . 'MB)',
                    __FILE__,
                    __LINE__
                );
            } else {
                $this->DebugMessage(
                    'bypassing EXIF and GetImageSize sections because $this->rawImageData is set, and $this->sourceFilename is not set, and source image is not too large for GD ('
                    . $this->source_width . 'x' . $this->source_width . '=' . ($this->source_width * $this->source_height * 5) . 'MB)',
                    __FILE__,
                    __LINE__
                );
            }
        ...
        $this->DebugMessage(
            'EXIF thumbnail extraction: (size=' . strlen($this->exif_thumbnail_data) . '; type="' . $this->exif_thumbnail_type . '"; ' . intval($this->exif_thumbnail_width) . 'x' . intval(
                $this->exif_thumbnail_height
            ) . ')',
            __FILE__,
            __LINE__
        ...
        if (($this->config_max_source_pixels > 0) && (($this->source_width * $this->source_height) > $this->config_max_source_pixels)) {
            ...
            $this->DebugMessage(
                'image is ' . $this->source_width . 'x' . $this->source_height . ' and therefore contains more pixels (' . ($this->source_width * $this->source_height)
    ...
    function SetCacheFilename()
    {
        ...
        if ($this->config_cache_default_only_suffix && $this->sourceFilename) {
            ...
            if (strpos($this->config_cache_default_only_suffix, '*') === false) {
                ...
                $this->cache_filename = $this->config_cache_directory . DIRECTORY_SEPARATOR . rawurlencode(str_replace('*', @$matches[1], $this->config_cache_default_only_suffix)) . '.' . strtolower(
    ...
    function ImageCreateFromFilename($filename)
    {
        ...
        if ($filename && ($getimagesizeinfo = @GetImageSize($filename))) {
            if (!$this->SourceImageIsTooLarge($getimagesizeinfo[0], $getimagesizeinfo[1])) {
                ...
                switch (@$getimagesizeinfo[2]) {
                    ...
                    case 15: // WBMP
                        ...
                        if (function_exists($ImageCreateFromFunctionName)) {
                            ...
                            $this->DebugMessage(
                                'NOT calling ' . $ImageCreateFromFunctionName . '(' . $filename . ') because !function_exists(' . $ImageCreateFromFunctionName . ')',
                                __FILE__,
                                __LINE__
                            );
                        }
                ...
                $this->DebugMessage(
                    'image is ' . $getimagesizeinfo[0] . 'x' . $getimagesizeinfo[1] . ' and therefore contains more pixels (' . ($getimagesizeinfo[0] * $getimagesizeinfo[1])
                    . ') than $this->config_max_source_pixels setting (' . $this->config_max_source_pixels . ')',
                    __FILE__,
                    __LINE__
                );
    ...
    function SourceImageToGD()
    {
        ...
        if (!$this->gdimg_source && $this->rawImageData) {
            ...
            if ($this->SourceImageIsTooLarge($this->source_width, $this->source_height)) {
                ...
                return $this->ErrorImage(
                    'Source image is too large (' . $this->source_width . 'x' . $this->source_height . ' = ' . number_format($this->source_width * $this->source_height / 1000000, 1) . 'Mpx, max='
                    . number_format($this->config_max_source_pixels / 1000000, 1) . 'Mpx) for GD creation (either install ImageMagick or increase PHP memory_limit to at least ' . ceil(
                        ($memory_get_usage + (5 * $this->source_width * $this->source_height)) / 1048576
                    ) . 'M).'
                );
            }
            ...
            if (!$this->gdimg_source) {
                ...
                $this->DebugMessage(
                    'SourceImageToGD.ImageCreateFromStringReplacement() failed with unknown image type "' . substr($this->rawImageData, 0, 4) . '" (' . phpthumb_functions::HexCharDisplay(
                        substr($this->rawImageData, 0, 4)
                    ) . ')',
                    __FILE__,
                    __LINE__
...
//				return $this->ErrorImage('Unknown image type identified by "'.substr($this->rawImageData, 0, 4).'" ('.phpthumb_functions::HexCharDisplay(substr($this->rawImageData, 0, 4)).') in SourceImageToGD()['.__LINE__.']');
        ...
        while (true) {
            ...
            if (ini_get('safe_mode')) {
                ...
                if (($this->w > $this->exif_thumbnail_width) || ($this->h > $this->exif_thumbnail_height)) {
                    $this->DebugMessage(
                        'Not using EXIF thumbnail data because EXIF thumbnail is too small (' . $this->exif_thumbnail_width . 'x' . $this->exif_thumbnail_height . ' vs ' . $this->w . 'x' . $this->h
                        . ')',
                        __FILE__,
                        __LINE__
                    );
        ...
        if (!$this->gdimg_source) {
            ...
            if ($imageHeader) {
                ...
                if ($this->config_error_die_on_source_failure) {
                    ...
                    if ($this->SourceImageIsTooLarge($this->getimagesizeinfo[0], $this->getimagesizeinfo[1])) {
                        $memory_get_usage = (function_exists('memory_get_usage') ? memory_get_usage() : 0);
                        $errormessages[]  = 'Source image is too large (' . $this->getimagesizeinfo[0] . 'x' . $this->getimagesizeinfo[1] . ' = ' . number_format(
                                $this->getimagesizeinfo[0] * $this->getimagesizeinfo[1] / 1000000,
                                1
                            ) . 'Mpx, max=' . number_format($this->config_max_source_pixels / 1000000, 1) . 'Mpx) for GD creation (either install ImageMagick or increase PHP memory_limit to at least '
                            . ceil(($memory_get_usage + (5 * $this->getimagesizeinfo[0] * $this->getimagesizeinfo[1])) / 1048576) . 'M).';
                    ...
                    $this->DebugMessage(
                        'All attempts to create GD image source failed (' . (ini_get('safe_mode') ? 'Safe Mode enabled, ImageMagick unavailable and source image probably too large for GD'
                            : ($GDreadSupport ? 'source image probably corrupt' : 'GD does not have read support for "' . $imageHeader . '"')) . '), cannot generate thumbnail'
                    );
            ...
            switch (@$this->getimagesizeinfo[2]) {
                ...
                case 8:
                    return $this->ErrorImage(
                        $this->ImageMagickVersion() ? 'ImageMagick failed on TIFF source conversion' : 'ImageMagick is unavailable and phpThumb() does not support TIFF source images without it'
                    );
                    break;
            ...
            if (!$this->gdimg_source) {
                ...
                if (!$this->ImageMagickVersion() && !phpthumb_functions::gd_version()) {
                    ...
                    return $this->ErrorImage(
                        $this->ImageMagickVersion() ? 'ImageMagick failed on WMF source conversion' : 'ImageMagick is unavailable and phpThumb() does not support WMF source images without it'
                    );
                } elseif ($HeaderFourBytes == '%PDF') { // "%PDF"

                    return $this->ErrorImage(
                        $this->ImageMagickVersion() ? 'ImageMagick and GhostScript are both required for PDF source images; GhostScript may not be properly configured'
                ...
                return $this->ErrorImage(
                    'Unknown image type identified by "' . $HeaderFourBytes . '" (' . phpthumb_functions::HexCharDisplay($HeaderFourBytes) . ') in SourceImageToGD() [' . __LINE__ . ']'
                );
            }
        }
    ...
    function phpThumbDebug($level = '')
    {
        ...
        foreach ($FunctionsExistance as $functionname) {
            $DebugOutput[] = 'builtin_function_exists(' . $functionname . ')' . str_repeat(' ', 23 - strlen($functionname)) . ' = ' . $this->phpThumbDebugVarDump(
                    phpthumb_functions::builtin_function_exists($functionname)
                );
        }
        $DebugOutput[] = '';
    ...
    function ImageCreateFromStringReplacement(&$RawImageData, $DieOnErrors = false)
    {
        ...
        switch (substr($RawImageData, 0, 3)) {
            ...
            default:
                $this->DebugMessage(
                    'ImageCreateFromStringReplacement() failed: unknown fileformat signature "' . phpthumb_functions::HexCharDisplay(substr($RawImageData, 0, 3)) . '"',
                    __FILE__,
                    __LINE__
                );
    ...
    function DebugTimingMessage($message, $file = '', $line = '', $timestamp = 0)
    {
        ...
        $this->debugtiming[number_format($timestamp, 6, '.', '')]
            = ': ' . $message . ($file ? ' in file "' . (basename($file) ? basename($file) : $file) . '"' : '') . ($line ? ' on line ' . $line : '');
