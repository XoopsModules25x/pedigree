...
class Zebra_Image
{
     ...
     *  require_once __DIR__ . '/path/to/Zebra_Image.php';
     *
            ...
            if ($this->_create_from_source()) {
                ...
                if (is_array($filter)) {
                    ...
                    foreach ($filter as $arguments) {
                        // if filter exists
                        if (defined('IMG_FILTER_' . mb_strtoupper($arguments[0]))) {
                            // try to apply the filter...
                            if (!@imagefilter(...array_merge([$target_identifier, constant('IMG_FILTER_' . mb_strtoupper($arguments[0]))], array_slice($arguments, 1)))) {
                                // ...and trigger an error if the filter could not be applied
                                trigger_error('Invalid arguments used for "' . mb_strtoupper($arguments[0]) . '" filter', E_USER_WARNING);
                            }
                            // if filter doesn't exists, trigger an error
                        } else {
                            trigger_error('Filter "' . mb_strtoupper($arguments[0]) . '" is not available', E_USER_WARNING);
                        }
                    }
                    // if a single filter is to be applied and it is available
                } elseif (defined('IMG_FILTER_' . mb_strtoupper($filter))) {
                    // get all the arguments passed to the method
                    $arguments = func_get_args();

                    // try to apply the filter...
                    if (!@imagefilter(...array_merge([$target_identifier, constant('IMG_FILTER_' . mb_strtoupper($filter))], array_slice($arguments, 1)))) {
                        // ...and trigger an error if the filter could not be applied
                        trigger_error('Invalid arguments used for "' . mb_strtoupper($arguments[0]) . '" filter', E_USER_WARNING);
                    }
                    // if filter doesn't exists, trigger an error
    ...
    public function resize($width = 0, $height = 0, $method = ZEBRA_IMAGE_CROP_CENTER, $background_color = '#FFFFFF')
    {
        ...
        if ($this->_create_from_source()) {
            ...
            // if
            if (// all images are to be resized - including images that are smaller than the given width/height
                ...
                // if
                if (// aspect ratio needs to be preserved AND
                    ...
                    // prepare the target image
                    $target_identifier = $this->_prepare_image(($width > 0 && $height > 0 && ZEBRA_IMAGE_NOT_BOXED != $method ? $width : $target_width), ($width > 0 && $height > 0 && ZEBRA_IMAGE_NOT_BOXED != $method ? $height : $target_height), $background_color);
    ...
    private function _create_from_source()
    {
        ...
        $this->target_type = mb_strtolower(mb_substr($this->target_path, mb_strrpos($this->target_path, '.') + 1));
    ...
    private function _write_image($identifier)
    {
        ...
        if ('' == $disabled_functions || false === mb_strpos('chmod', $disabled_functions)) {
            // chmod the file
